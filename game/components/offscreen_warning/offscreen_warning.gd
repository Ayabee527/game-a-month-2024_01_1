...
@export var draw_dist: float = 104.0
...
func _draw() -> void:
	if not seen:
		var dir_from_player: Vector2 = (
			MainCam.global_position.direction_to(global_position)
		)
		var dist_from_player: float = (
			MainCam.global_position.distance_to(global_position)
		)
		var warn_position := dir_from_player * 104.0
		...
		draw_circle(
			(MainCam.global_position - global_position) + warn_position,
